...
| allowCrsTextInput | boolean | true | If the user can input an coordinate reference system (crs) in a text input field. This data is then transformed into this crs for export. The user can either provide just the code (e.g. `25832`) or with EPSG: prefix (e.g. `EPSG:25832`). |
...
| crs | string | null | The output crs as epsg code. If array is provided user can select. Example: ["EPSG:25832", "EPSG:4326"]. If not provided, and crs text input is not allowed, the source crs is used. |
...
| dataProjection | [ProjectionOptions] | null | The source datas projection. If not provided, the default projection of the VC Map core is used. |
...
| --- | ---- | ----------- |
| epsg | string &vert; number | EPSG of the projection, for example: "EPSG:4326". If not specified, uses the framework projection. |
| proj4 | string | definition of the projection. See for example: http://spatialreference.org/ref/epsg/4326/proj4/
